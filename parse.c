...
Type *type_short = &(Type) {
	KIND_SHORT, 2, 2, false
};
Type *type_int = &(Type) {
	KIND_INT, 4, 4, false
};
...
Type *type_llong = &(Type) {
	KIND_LLONG, 8, 8, false
};
Type *type_uchar = &(Type) {
	KIND_CHAR, 1, 1, true
};
Type *type_ushort = &(Type) {
	KIND_SHORT, 2, 2, true
};
Type *type_uint = &(Type) {
	KIND_INT, 4, 4, true
};
Type *type_ulong = &(Type) {
	KIND_LONG, 8, 8, true
};
Type *type_ullong = &(Type) {
	KIND_LLONG, 8, 8, true
};
Type *type_float = &(Type) {
	KIND_FLOAT, 4, 4, false
};
Type *type_double = &(Type) {
	KIND_DOUBLE, 8, 8, false
};
Type *type_ldouble = &(Type) {
	KIND_LDOUBLE, 8, 8, false
};
Type *type_enum = &(Type) {
	KIND_ENUM, 4, 4, false
};
...
char *make_tempname() {
	static int c = 0;
	return format(".TEMPNAME_%d", c++);
}

char *make_label() {
	static int c = 0;
	return format(".LABEL_%d", c++);
}

static char *make_static_label(char *name) {
	static int c = 0;
	return format(".STATICLABEL_%d.%s", c++, name);
}
...
static Node *ast_gvar(Type *ty, char *name) {
	Node *r = make_ast(&(Node) {
		AST_GVAR, ty, .varname = name, .glabel = name
	});
...
static Type* make_ptr_type(Type *ty) {
	return make_type(&(Type) {
		KIND_PTR, .ptr = ty, .size = 8, .align = 8
	});
}
...
static Type* make_rectype(bool is_struct) {
	return make_type(&(Type) {
		KIND_STRUCT, .is_struct = is_struct
	});
}
...
static Node *read_sizeof_operand_sub_expr() {
	Token *tok = get();
...
static Type *read_sizeof_operand_sub() {
	return read_sizeof_operand_sub_expr()->ty;
}

static Node *read_sizeof_operand() {
	Node *nd = read_sizeof_operand_sub_expr();
	// Sizeof on void or function type is GNU extension

	if (nd->ty->kind == KIND_FUNC) {
		Node* func_node = map_get(globalenv, "__sizeof_func");
		return ast_funcall(func_node->ty, func_node->fname, make_vector1(nd->sval));
	}

	int size = (nd->ty->kind == KIND_VOID) ? 1 : nd->ty->size;
	assert(0 <= size);
...
static Vector *read_func_args(Vector *params) {
	...
	for (;;) {
		...
		if (arg == NULL) {
			Token* tt = get();
			errort(tt, "invalid argument: '%s'", tok2s(tt));
		}
...
void parse_init() {
	...
	Vector* charptr = make_vector1(make_ptr_type(type_char));

	Vector *two_voidptrs = make_vector();
	vec_push(two_voidptrs, make_ptr_type(type_void));
	vec_push(two_voidptrs, make_ptr_type(type_void));

	define_builtin("__asm", type_void, charptr);
	define_builtin("__sizeof_func", type_ulong, charptr);
